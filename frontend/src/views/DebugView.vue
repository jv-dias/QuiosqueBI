  ...
  <main class="container mx-auto py-10 px-4">
    ...
    <div class="max-w-2xl mx-auto p-8 bg-gray-50 rounded-lg shadow-md mb-12">
      ...
      <div class="space-y-6">
        <div>
          ...
          <div class="mt-1">
            <input id="debug-file-upload" name="debug-file-upload" type="file" accept=".csv" @change="handleFileChangeDebug" class="block w-full text-sm text-slate-500
    ...
    <div v-else-if="debugInfo" class="p-8 bg-white rounded-lg shadow-md">
      ...
      <div class="mb-6 p-4 bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <pre class="bg-gray-100 p-3 rounded-md text-sm whitespace-pre-wrap">{{ JSON.stringify(debugInfo.planoDaIA, null, 2) }}</pre>
      </div>

      <div class="mb-6 p-4 bg-white rounded-lg shadow-sm border border-gray-200">
        <h4 class="font-bold text-lg text-gray-800 mb-2">Cabeçalhos Encontrados no Arquivo:</h4>
        <pre class="bg-gray-100 p-3 rounded-md text-sm whitespace-pre-wrap">{{ JSON.stringify(debugInfo.cabecalhosDoArquivo, null, 2) }}</pre>
      </div>
      ...
      <div class="p-4 bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <p v-if="debugInfo.dadosBrutosDoArquivo.length === 0" class="text-center text-gray-500 mt-4">Nenhum dado bruto para exibir.</p>
      </div>
    </div>
  </main>
</template>